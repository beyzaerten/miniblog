...
// super: extend edilen classın constructorı
// bloc açıldığında initial durumunda açılır. Bu yüzden superın içine ArticlesInitial yazdık
class ArticleBloc extends Bloc<ArticleEvent, ArticleState> {
  ...
  // emitter => statei değiştiren mekanizma
  ...
  //   try {
  //     final article = await articleRepository.fetcBlogById(event.id);
  ...
  void _onAddArticle(AddArticle event, Emitter<ArticleState> emit) async {
    ...
    try {
      ...
      emit(ArticlesLoaded(blogs: addArticle));
    } catch (e) {
      emit(ArticlesError());
    }
  }
